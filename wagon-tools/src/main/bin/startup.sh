...
base=`pwd`
cd $bin_path
sync_conf=$base/conf/sync.properties
...
SYNC_OPTS="-Dlogback.configurationFile=$logback_configurationFile -Dsync.conf=$sync_conf -DappName=order_sync"

if [ -e $sync_conf -a -e $logback_configurationFile ]
then

    	MAIN_CLASS="com.youzan.wagon.binlog.MysqlBinlogParserLauncher"
    	for i in $base/lib/*;
        ...
        $JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $SYNC_OPTS -classpath $CLASSPATH $MAIN_CLASS 1>>$base/logs/sync/binlog.log 2>&1 &
        echo $! > $base/bin/sync.pid

        echo "cd to $bin_path for continue"
        cd $bin_path
else
        echo "canal conf("$sync_conf") OR log configration file($logback_configurationFile) is not exist,please create then first!"
fi