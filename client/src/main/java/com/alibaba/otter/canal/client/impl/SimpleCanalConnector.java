...
public class SimpleCanalConnector implements CanalConnector {
    ...
    // =============================== extend command ========================================
    public CanalDBSwitchResult canalDBSwitch(CanalDBSwitchRequestData data) throws CanalClientException {
        waitClientRunning();
        String json = data.toJson(); // 消息体，json字符串
        
        // new Packet
        CmdExtend cmdExtend = CmdExtend.newBuilder()
                .setCmdType(CmdExtendType.CANAL_DB_SWITCH)
                .setBody(json).build();
        ...
        try {
            // send
            writeWithHeader(channel, packet.toByteArray());
            logger.info("send canal db switch request successful, address:{}, reequest data:{}", address, json);
            
            // 获取结果
            CmdExtend respCmd = receiveExtendCmdResults();
            if (!CmdExtendType.CANAL_DB_SWITCH_RESULT.equals(respCmd.getCmdType())) {
                throw new CanalClientException("the response command type is not {}.", CmdExtendType.CANAL_DB_SWITCH_RESULT);
            }
            return CanalDBSwitchResult.fromJson(respCmd.getBody(), CanalDBSwitchResult.class);
        } catch (Exception e) {
            throw new CanalClientException(e);
        }
    }

    public CanalServerRunningData queryCanalRunningInfo(String destination) throws CanalClientException {
        waitClientRunning();
        
        CmdExtend cmdExtend = CmdExtend.newBuilder()
                .setCmdType(CmdExtendType.QUERY_CANAL_RUNNING_INFO)
                .setBody(destination).build();
        Packet packet = Packet.newBuilder()
                .setType(PacketType.CMDEXTEND)
                .setBody(cmdExtend.toByteString()).build();
        try {
            writeWithHeader(channel, packet.toByteArray());
            logger.info("send query canal instance running info request successful, destination={}", destination);
            
            // 获取结果
            CmdExtend respCmd = receiveExtendCmdResults();
            if (!CmdExtendType.QUERY_CANAL_RUNNING_INFO_RESULT.equals(respCmd.getCmdType())) {
                throw new CanalClientException("the command response is not query_canal_instance_running_info_result.");
            }
            CanalServerRunningData switchResults = CanalServerRunningData.fromJson(respCmd.getBody(), CanalServerRunningData.class);
    ...
    public TablePositionData queryTablePosition(String tableName) throws CanalClientException {
        waitClientRunning();
        
        CmdExtend cmdExtend = CmdExtend.newBuilder()
                .setCmdType(CmdExtendType.QUERY_TABLE_POSITION)
                .setBody(tableName).build();
        Packet packet = Packet.newBuilder()
                .setType(PacketType.CMDEXTEND)
                .setBody(cmdExtend.toByteString()).build();
        try {
            writeWithHeader(channel, packet.toByteArray());
            logger.info("send query table position request successful, tableName={}", tableName);
            
            // 获取结果
            CmdExtend respCmd = receiveExtendCmdResults();
            if (!CmdExtendType.QUERY_TABLE_POSITION_RESULT.equals(respCmd.getCmdType())) {
                throw new CanalClientException("the command response is not query_canal_instance_running_info_result.");
            }
            TablePositionData switchResults = TablePositionData.fromJson(respCmd.getBody(), TablePositionData.class);
            return switchResults;
        } catch (IOException e) {
            throw new CanalClientException(e);
        }
    ...
    public TablePositionData queryTablePosition_bak(String tableName) throws CanalClientException {
        waitClientRunning();
        ...
        try {
            writeWithHeader(channel, packet.toByteArray());
            logger.info("send query table position request successful, destination={}", tableName);
            
            // 获取结果
            CmdExtend respCmd = receiveExtendCmdResults();
            if (!CmdExtendType.QUERY_TABLE_POSITION_RESULT.equals(respCmd.getCmdType())) {
                throw new CanalClientException("the command response is not query_table_position_result.");
            }
            return TablePositionData.fromJson(respCmd.getBody(), TablePositionData.class);
        } catch (IOException e) {
            throw new CanalClientException(e);
        }
    }
    
    private CmdExtend receiveExtendCmdResults() throws InvalidProtocolBufferException, IOException {
        Packet p = Packet.parseFrom(readNextPacket(channel));
        switch (p.getType()) {
            case CMDEXTEND: {
                if (!p.getCompression().equals(Compression.NONE)) {
                ...
                return CanalPacket.CmdExtend.parseFrom(p.getBody());
            }
            default: {
                throw new CanalClientException("unexpected packet type: " + p.getType());
            }
